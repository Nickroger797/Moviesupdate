...
async def start(client:Client, message): 
            ...
            if refData and refData.split("-", 1)[0] == "biisal":
                ...
                if AUTH_CHANNEL and await is_req_subscribed(client, message):
                        buttons = [[
                            ...
                            [
                            InlineKeyboardButton('👻 Read Carefully 👀', callback_data=f'point#{message.from_user.id}'),
    ...
    if AUTH_CHANNEL and not await is_req_subscribed(client, message):
        ...
        await client.send_message(
            ...
            text="<b>🙁 ғɪʀꜱᴛ ᴊᴏɪɴ ᴏᴜʀ ʙᴀᴄᴋᴜᴘ ᴄʜᴀɴɴᴇʟ ᴛʜᴇɴ ʏᴏᴜ ᴡɪʟʟ ɢᴇᴛ ᴍᴏᴠɪᴇ\n🧊ᴏᴛʜᴇʀᴡɪꜱᴇ ʏᴏᴜ ᴡɪʟʟ ɴᴏᴛ ɢᴇᴛ ɪᴛ.\n\nᴄʟɪᴄᴋ ᴊᴏɪɴ ɴᴏᴡ ʙᴜᴛᴛᴏɴ 👇</b>",
            reply_markup=InlineKeyboardMarkup(btn),
            parse_mode=enums.ParseMode.HTML
        )
        return
    ...
    if not await db.has_premium_access(user_id):
        ...
        if settings.get("is_verify", IS_VERIFY) and not user_verified or is_second_shortener or is_third_shortener:
            ...
            buttons = [[
                InlineKeyboardButton(text="🧊 ᴠᴇʀɪғʏ 🎗️", url=verify)
            ],[
...
async def settings(client, message):
    ...
    if settings is not None:
            buttons = [[
                InlineKeyboardButton('♟️ᴀᴜᴛᴏ ꜰɪʟᴛᴇʀ♟️', callback_data=f'setgs#auto_filter#{settings["auto_filter"]}#{grp_id}'),
...
async def send_msg(bot, message):
    ...
    if message.reply_to_message:
        ...
        try:
            ...
            await message.reply_text(f"<b>🔋 ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟʟʏ ᴍᴇꜱꜱᴀɢᴇ ꜱᴇɴᴛ ɪɴ `{success_count}` ɪᴅ\n<code>{out}</code></b>")
        except Exception as e:
            await message.reply_text(f"<b>🎭 ᴇʀʀᴏʀ - <code>{e}</code></b>")
    else:
...
async def send_request(bot, message):
    try:
        ...
        await message.reply_text("<b>🪩 ʏᴏᴜʀ ʀᴇǫᴜᴇsᴛ ɪs ɪɴᴄᴏᴍᴘʟᴇᴛᴇ</b>")
        return
    buttons = [[
...
async def search_files(bot, message):
    ...
    await message.reply_document(
        ...
        caption=f"<b>🙄 ʙʏ ʏᴏᴜʀ ꜱᴇᴀʀᴄʜ, ɪ ꜰᴏᴜɴᴅ - <code>{total}</code> ꜰɪʟᴇs</b>",
        parse_mode=enums.ParseMode.HTML
    )
...
async def save_caption(client, message):
    ...
    await message.reply_text(f"🌪️Successfully changed caption for {title} to\n\n{caption}", disable_web_page_preview=True) 
...
async def save_tutorial(client, message):
    ...
    try:
        ...
        return await message.reply_text("<b>Command Incomplete!!\n\nuse like this -</b>\n\n<code>/set_caption https://t.me/Ott_Sandhu</code>")    
...
async def set_shortner(c, m):
    ...
    try:
        URL = m.command[1]
        API = m.command[2]
        resp = requests.get(f'https://{URL}/api?api={API}&url=https://telegram.dog/Ott_Sandhu').json()
...
async def set_shortner_2(c, m):
    ...
    try:
        ...
        await m.reply_text(f"<b><u>🎗️ sᴜᴄᴄᴇssꜰᴜʟʟʏ ʏᴏᴜʀ sʜᴏʀᴛɴᴇʀ ɪs ᴀᴅᴅᴇᴅ</u>\n\nᴅᴇᴍᴏ - {SHORT_LINK}\n\nsɪᴛᴇ - `{URL}`\n\nᴀᴘɪ - `{API}`</b>", quote=True)
        user_id = m.from_user.id
        user_info = f"@{m.from_user.username}" if m.from_user.username else f"{m.from_user.mention}"
        link = (await c.get_chat(m.chat.id)).invite_link
        grp_link = f"[{m.chat.title}]({link})"
...
🌪️ ᴛᴜᴛᴏʀɪᴀʟ ʟɪɴᴋ - {settings['tutorial']}
...
🔮 ꜰɪʟᴇ ᴄᴀᴘᴛɪᴏɴ - `{settings['caption']}`</b>"""
    ...
    btn = [[
        InlineKeyboardButton("☄️ʀᴇꜱᴇᴛ ᴅᴀᴛᴀ", callback_data="reset_grp_data")
    ],[
        InlineKeyboardButton("👻ᴄʟᴏsᴇ👻", callback_data="close_data")
    ]]
    reply_markup=InlineKeyboardMarkup(btn)
...
async def set_shortner_3(c, m):
    ...
    try:
        ...
        await m.reply_text(f"<b><u>😵‍💫 ᴇʀʀᴏʀ ᴏᴄᴄᴏᴜʀᴇᴅ!!</u>\n\🧞nᴀᴜᴛᴏ ᴀᴅᴅᴇᴅ ʙᴏᴛ ᴏᴡɴᴇʀ ᴅᴇꜰᴜʟᴛ sʜᴏʀᴛɴᴇʀ\n\n🫧ɪꜰ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴄʜᴀɴɢᴇ ᴛʜᴇɴ ᴜsᴇ ᴄᴏʀʀᴇᴄᴛ ꜰᴏʀᴍᴀᴛ ᴏʀ ᴀᴅᴅ ᴠᴀʟɪᴅ sʜᴏʀᴛʟɪɴᴋ ᴅᴏᴍᴀɪɴ ɴᴀᴍᴇ & ᴀᴘɪ\n\n🌪️ʏᴏᴜ ᴄᴀɴ ᴀʟsᴏ ᴄᴏɴᴛᴀᴄᴛ ᴏᴜʀ <a href=https://t.me/codexdisscus>💞sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ💞</a> ꜰᴏʀ sᴏʟᴠᴇ ᴛʜɪs ɪssᴜᴇ...\n\n🔮ʟɪᴋᴇ -\n\n`/set_shortner_3 mdiskshortner.link e7beb3c8f756dfa15d0bec495abc65f58c0dfa95`\n\n💔 ᴇʀʀᴏʀ - <code>{e}</code></b>", quote=True)
