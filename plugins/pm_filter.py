...
async def next_page(bot, query):
    ...
    btn.insert(1, [
        ...
        InlineKeyboardButton("🪄 ʏᴇᴀʀ 🎗️", callback_data=f"years#{key}#{offset}#{req}"),
    ])
    btn.insert(2, [
...
async def season_search(client: Client, query: CallbackQuery):
    ...
    btn.insert(1, [
        InlineKeyboardButton("💧 ǫᴜᴀʟɪᴛʏ 💀", callback_data=f"qualities#{key}#{offset}#{req}"),
        InlineKeyboardButton("🌬️ ʏᴇᴀʀ ⌛", callback_data=f"years#{key}#{offset}#{req}"),
    ])
    btn.insert(2,[
    ...
    if n_offset== '':
        ...
        btn.append(
            [InlineKeyboardButton("😶‍🌫️ ʙᴀᴄᴋ", callback_data=f"season_search#{season}#{key}#{offset- int(MAX_BTN)}#{orginal_offset}#{req}"),
...
async def year_search(client: Client, query: CallbackQuery):
    ...
    btn.insert(1, [
        InlineKeyboardButton("🫠 ǫᴜᴀʟɪᴛʏ 💦", callback_data=f"qualities#{key}#{offset}#{req}"),
        InlineKeyboardButton("🙂 ʟᴀɴɢᴜᴀɢᴇ 🌬️", callback_data=f"languages#{key}#{offset}#{req}"),
    ])
    btn.insert(2,[
        InlineKeyboardButton("⛄ sᴇɴᴅ ᴀʟʟ", callback_data=batch_link),
        ])
    ...
    if n_offset== '':
        ...
        btn.append(
            [InlineKeyboardButton("🏝️ ʙᴀᴄᴋ", callback_data=f"years_search#{year}#{key}#{offset- int(MAX_BTN)}#{orginal_offset}#{req}"),
             InlineKeyboardButton(f"{math.ceil(offset / int(MAX_BTN)) + 1}/{math.ceil(total / int(MAX_BTN))}", callback_data="pages",),
             InlineKeyboardButton("ɴᴇxᴛ 💦", callback_data=f"years_search#{year}#{key}#{n_offset}#{orginal_offset}#{req}"),])
...
async def quality_search(client: Client, query: CallbackQuery):
    ...
    btn.insert(1, [
        InlineKeyboardButton("🌪️ season🍿", callback_data=f"seasons#{key}#{offset}#{req}"),
    ...
    if n_offset== '':
        ...
        btn.append(
            [InlineKeyboardButton("✳️ ʙᴀᴄᴋ", callback_data=f"quality_search#{qul}#{key}#{offset- int(MAX_BTN)}#{orginal_offset}#{req}"),
             InlineKeyboardButton(f"{math.ceil(offset / int(MAX_BTN)) + 1}/{math.ceil(total / int(MAX_BTN))}", callback_data="pages",),
            ])
    elif offset==0:
        btn.append(
    ...
    btn.append([
        InlineKeyboardButton(text="🌪️ ʙᴀᴄᴋ ᴛᴏ ᴍᴀɪɴ ᴘᴀɢᴇ", callback_data=f"next_{req}_{key}_{orginal_offset}"),])
    await query.message.edit_text(cap + links + del_msg, disable_web_page_preview=True, parse_mode=enums.ParseMode.HTML, reply_markup=InlineKeyboardMarkup(btn))
    return
...
async def advantage_spoll_choker(bot, query):
    ...
    await query.answer('ᴄʜᴇᴄᴋɪɴɢ ɪɴ ᴍʏ ᴅᴀᴛᴀʙᴀꜱᴇ ☠️')
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        reply_markup = InlineKeyboardMarkup(btn)
        return await query.message.edit(text=f'<b>ᴛʜᴀɴᴋs ғᴏʀ ᴊᴏɪɴɪɴɢ ᴏᴜʀ ᴄʜᴀɴɴᴇʟ 🌪️\nɢᴇᴛ ʏᴏᴜʀ ғɪʟᴇ : {files.file_name[:20]}.. ʙʏ ᴄʟɪᴄᴋɪɴɢ ᴛʜᴇ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ 🧨\n\nJᴀɪ sʜʀᴇᴇ RAM 🙏</b>',reply_markup=reply_markup)
        ...
        await query.answer(f"ʏᴏᴜ sᴇʟᴇᴄᴛᴇᴅ {lang.title()} ʟᴀɴɢᴜᴀɢᴇ 🍁", show_alert=True)
        ...
        [
            InlineKeyboardButton('👀 Read Carefully 👀', callback_data=f'point#{query.from_user.id}'),
        ...
        #add back button
        buttons = [[
            InlineKeyboardButton('✳️ ᴀᴅᴅ ᴍᴇ ᴛᴏ ʏᴏᴜʀ ɢʀᴏᴜᴘs ✳️', url=f'http://t.me/{temp.U_NAME}?startgroup=start')],
            [InlineKeyboardButton('🫧 ʙᴀᴄᴋ', callback_data='features')]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        try:
            ...
            await query.answer("🙄 ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀ ᴍᴇᴍʙᴇʀ ᴏꜰ ᴛʜɪꜱ ᴄʜᴀɴɴᴇʟ, ꜰɪʀꜱᴛ ᴊᴏɪɴ", show_alert=True)
        ...
        userid = query.from_user.id
        buttons = [[
            InlineKeyboardButton("🌬️ ᴜᴘʟᴏᴀᴅ ɪɴ 1 ʜᴏᴜʀꜱ 🌬️", callback_data=f"upload_alert#{user_id}")
        ]]
        btn = [[
            InlineKeyboardButton("🌪️ ᴠɪᴇᴡ sᴛᴀᴛᴜs 💧", url=f"{query.message.link}")
        ]]
        st = await client.get_chat_member(chnl_id, userid)
        if (st.status == enums.ChatMemberStatus.ADMINISTRATOR) or (st.status == enums.ChatMemberStatus.OWNER):
            user = await client.get_users(user_id)
        ...
        btn = [[
            InlineKeyboardButton("🌀 ᴠɪᴇᴡ sᴛᴀᴛᴜs 🌪️", url=f"{query.message.link}")
        ]]
        st = await client.get_chat_member(chnl_id, userid)
        if (st.status == enums.ChatMemberStatus.ADMINISTRATOR) or (st.status == enums.ChatMemberStatus.OWNER):
            user = await client.get_users(user_id)
            ...
            await query.message.edit_reply_markup(InlineKeyboardMarkup(buttons))
            try:
                await client.send_message(chat_id=user_id, text="<b>🌪️ʙʀᴏ ᴘʟᴇᴀꜱᴇ ᴛᴇʟʟ ᴍᴇ ʏᴇᴀʀꜱ ᴀɴᴅ ʟᴀɴɢᴜᴀɢᴇ, ᴛʜᴇɴ ɪ ᴡɪʟʟ ᴜᴘʟᴏᴀᴅ 😬</b>", reply_markup=InlineKeyboardMarkup(btn))
            except UserIsBlocked:
        ...
        userid = query.from_user.id
        if str(userid) in user_id:
            await query.answer("🌬️ʏᴏᴜʀ ʀᴇǫᴜᴇꜱᴛ ᴡɪʟʟ ʙᴇ ᴜᴘʟᴏᴀᴅᴇᴅ ᴡɪᴛʜɪɴ 1 ʜᴏᴜʀ 😁", show_alert=True)
        else:
            await query.answer(script.ALRT_TXT, show_alert=True)
        ...
        userid = query.from_user.id
        if str(userid) in user_id:
            await query.answer("🌬️ʙʀᴏ ᴘʟᴇᴀꜱᴇ ᴛᴇʟʟ ᴍᴇ ʏᴇᴀʀꜱ ᴀɴᴅ ʟᴀɴɢᴜᴀɢᴇ, ᴛʜᴇɴ ɪ ᴡɪʟʟ ᴜᴘʟᴏᴀᴅ 😬", show_alert=True)
        else:
            await query.answer(script.ALRT_TXT, show_alert=True)
...
async def auto_filter(client, msg, spoll=False , pm_mode = False):
    ...
    if offset != "":
        ...
        btn.insert(1,[
            InlineKeyboardButton("🏝️No More Pages🏝️", user_id=ADMINS[0])
        ])
